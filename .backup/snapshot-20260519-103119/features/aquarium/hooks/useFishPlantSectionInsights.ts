...
export function useFishPlantSectionInsights({
  ...
  const resolveFishCatalogEntryId = (item: any) => {
    const explicitId = String(item?.id ?? item?.catalogFishId ?? '').trim();
    if (explicitId) {
      return explicitId;
    }
    ...
    if (latinKey) {
      return `fish_${latinKey}`;
    }

    const commonKey = normalizeLatinCatalogKey(item?.commonName ?? item?.name);
    return commonKey ? `fish_${commonKey}` : '';
  };
  ...
  useEffect(() => {
    ...
    const selectedStillVisible = safeFilteredFishCatalog.some(
      (item) => resolveFishCatalogEntryId(item) === selectedCatalogFishId
    );
