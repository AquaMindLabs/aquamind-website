...
function evaluateStockingCompatibilityModel(aquarium, speciesList) {
  ...
  if (
    ...
    addIssue({
      ...
      severity: 'warning',
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message: 'Brak wspolnego zakresu KH dla gatunkow, ktore wymagaja KH.',
    });
  }
    ...
    for (let compareIndex = index + 1; compareIndex < normalizedSpecies.length; compareIndex += 1) {
      ...
      if (
        ...
        addIssue({
          ...
          speciesIds: pairIds,
          message: `${firstName} i ${secondName}: mozliwy konflikt terytorialny w tej samej strefie.`,
        });
      }
      ...
      if (
        ...
        addIssue({
          ...
          severity: 'error',
          speciesIds: pairIds,
          message: `${firstName} moze potraktowac ${secondName} jako pokarm.`,
        });
      }
      if (
        ...
        addIssue({
          ...
          severity: 'error',
          speciesIds: pairIds,
          message: `${secondName} moze potraktowac ${firstName} jako pokarm.`,
        });
      }
      ...
      if (first.profile.mayEatShrimp && second.kind === 'shrimp') {
        ...
        addIssue({
          ...
          speciesIds: pairIds,
          message: `${firstName} moze stanowic ryzyko dla krewetek (${secondName}).`,
        });
      }
      if (second.profile.mayEatShrimp && first.kind === 'shrimp') {
        ...
        addIssue({
          ...
          severity: highRisk ? 'error' : 'warning',
          speciesIds: pairIds,
          message: `${secondName} moze stanowic ryzyko dla krewetek (${firstName}).`,
        });
      }
      ...
      if (
        ...
        addIssue({
          ...
          severity: 'warning',
          speciesIds: pairIds,
          message: `${firstName} i ${secondName}: mozliwe podgryzanie pletw.`,
        });
      }
    }
  }
  ...
  if (bottomPressure >= 12 || middlePressure >= 14) {
    ...
    addIssue({
      ...
      severity: 'warning',
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message: 'Wysoka konkurencja o strefy plywania moze podnosic stres i konflikty.',
    });
  }
  if (territorialByZone.bottom >= 6 && aquariumLengthCm !== null && aquariumLengthCm < 90) {
    ...
    addIssue({
      ...
      severity: 'warning',
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message: 'Wiele terytorialnych gatunkow dennych w ograniczonej przestrzeni dna.',
    });
  }
  ...
  if (hasMarine && hasFresh) {
    ...
    addIssue({
      ...
      severity: 'error',
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message: 'Nie mozna laczyc gatunkow morskich ze slodkowodnymi.',
    });
  }
  if (hasBrackish && hasFresh && !hasMarine) {
    ...
    addIssue({
      ...
      severity: 'error',
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message: 'Nie mozna laczyc gatunkow slonawowodnych ze slodkowodnymi.',
    });
  }
  if (preferences.includes('soft_acidic') && preferences.includes('hard_alkaline')) {
    ...
    addIssue({
      ...
      speciesIds: normalizedSpecies.map((entry) => entry.speciesId).filter(Boolean),
      message:
        'Parametry tych gatunkow sa mocno rozne: miekka/kwasna vs twarda/zasadowa woda.',
    });
  }
  ...
  if (!hasHidingPlaces) {
    ...
    if (aggressiveSpecies.length > 0) {
      ...
      addIssue({
        ...
        message: 'Brak kryjowek przy obsadzie terytorialnej/agresywnej zwieksza ryzyko konfliktow.',
      });
    }
  }
  ...
  issues.forEach((issue) => {
    if (issue.type === 'schooling') {
      uniquePush(recommendations, 'Zwieksz grupe do minimum zalecanego dla gatunku.');
    }
    if (issue.type === 'tank_size') {
      uniquePush(recommendations, 'Rozwaz wieksze akwarium lub mniejsza obsade.');
    }
    if (issue.type === 'shrimp_risk') {
      uniquePush(recommendations, 'Nie lacz ryzykownych gatunkow z krewetkami.');
    }
    ...
    if (issue.type === 'territory') {
      uniquePush(recommendations, 'Zwieksz liczbe kryjowek i ogranicz gatunki konkurujace o te same strefy.');
    }
  });
  ...
  const summary =
    overallStatus === 'compatible'
      ...
      : overallStatus === 'caution'
        ? 'Obsada wymaga ostroznosci - widoczne sa czynniki ryzyka.'
        : overallStatus === 'unknown'
