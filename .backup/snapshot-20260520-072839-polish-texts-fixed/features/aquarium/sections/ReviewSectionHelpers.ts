...
export function buildTrendSuggestedEnvironmentForTank(
  ...
  if (activePlantDiseaseIds.size > 0 && treatmentReasons.length === 0) {
    recommendedLightRange = { min: 6, max: 8, conflict: false };
    treatmentReasons.push("aktywny problem roślin");
  }
...
export function buildAttentionItemsForTank(
  ...
  if (
    ...
    appendItem({
      ...
      explanation: "Bez czasu świecenia trudniej ocenic przyczyny słabego wzrostu roślin lub glonów.",
    ...
    appendItem({
      ...
      explanation: "Nieoptymalny fotoperiod może nasilic glony lub oslabic rośliny.",
  ...
  if (incompatibleFishCount > 0) {
    appendItem({
      ...
      title: fishMismatchNames
        ? `Niedopasowanie warunków u ryb: ${fishMismatchNames}.`
        : `Wykryto niezgodnosci dla ryb (${incompatibleFishCount} gat.).`,
      explanation: "Część ryb ma wymagania odbiegajace od aktualnych warunków akwarium.",
  ...
  if (fishAggressionConflictsCount > 0) {
    appendItem({
      ...
      explanation: "Konflikty behawioralne mogą prowadzic do przewleklego stresu i urazow.",
  ...
  if (fishStockingSummary.hasFish && !fishStockingSummary.hasTankLiters) {
    ...
    appendItem({
      severity: 'warning',
      title: `Obsada jest lekko za duża (${Math.round(
        fishStockingSummary.ratio * 100
  ...
  if (incompatiblePlantCount > 0) {
    appendItem({
      ...
      title: plantMismatchNames
        ? `Niedopasowanie warunków u roślin: ${plantMismatchNames}.`
        : `Wykryto niezgodnosci dla roślin (${incompatiblePlantCount} gat.).`,
      explanation: "Część roślin ma wymagania, które są poza aktualnymi warunkami.",
  ...
  if (activeIssueCasesCount > 0) {
    appendItem({
      ...
      details: [
        ...
        `Choroby roślin: ${activePlantDiseaseCasesCount}.`,
    ...
    (selectedTankHealthAssessment.penalties ?? [])
      ...
      .forEach((penalty: any) =>
        appendItem({
          ...
          suggestedAction: "Wprowadź poprawke i sprawdź zmiane po kolejnym pomiarze.",
          source: 'health_score',
          affectedArea: 'general',
          area: 'Ogolne',
...
export function buildHomeSectionCounts(
  ...
  const plantCompatibilityResults = plantItems.map((item: any) => ({
    id: item.id,
    label: `${item.commonName ?? item.name ?? item.latinName ?? "Roślina"} (${item.latinName ?? 'brak nazwy lacinskiej'})`,
