...
export function useReviewSectionInsights({
  ...
  const plantCareTips = useMemo(() => {
    ...
    if (lowNutrients.length > 0) {
      tips.push(
        "Możliwy niedobor skladnikow odżywczych. Jak to zrobic: startuj od ok. 1/3 dawki producenta 2-3 razy w tygodniu, po 7 dniach ocen rośliny i glony, potem zwiększaj dawke o 10-20% tygodniowo az do stabilnego wzrostu."
      );
    }
    ...
    if (co2 !== null && co2 < co2Min) {
      tips.push(
        "CO2 jest za niskie dla stabilnego wzrostu roślin. Jak to zrobic: podnos podawanie bardzo stopniowo (ok. 10-15% co 2-3 dni), uruchamiaj CO2 1-2 h przed swiatlem i obserwuj ryby; przy oznakach dusznosci natychmiast zmniejsz dawke i zwiększ ruch tafli."
      );
    }
