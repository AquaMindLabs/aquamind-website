...
function buildFallbackId(name) {
  const slug = normalizeText(name)
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
  return `fish_disease_${slug || 'unknown'}`;
}

async function loadDiseaseCatalog(repoRoot) {
  const diseaseUrl = pathToFileURL(
    path.join(repoRoot, 'data', 'diseaseCatalog.js')
  ).href;
  const diseaseModule = await import(diseaseUrl);
  const records = Array.isArray(diseaseModule.DISEASE_CATALOG)
    ? diseaseModule.DISEASE_CATALOG
    : [];
...
async function syncFishDiseaseCatalog({ db, records, dryRun, prune }) {
  const collectionName = 'fishDiseaseCatalog';
...
async function main() {
  ...
  const records = await loadDiseaseCatalog(repoRoot);
  const result = await syncFishDiseaseCatalog({
    db,
    records,
    dryRun: flags.dryRun,
    prune: flags.prune,
