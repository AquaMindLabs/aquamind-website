...
type TankContextValue = {
  ...
  subscriptionPriceByTier: Partial<Record<SubscriptionTier, string>>;
...
export function TankProvider({ children }: TankProviderProps) {
  ...
  const [subscriptionPriceByTier, setSubscriptionPriceByTier] = useState<
    Partial<Record<SubscriptionTier, string>>
  >({});
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const nextPriceMap: Partial<Record<SubscriptionTier, string>> = {};
        (['premium', 'pro'] as SubscriptionTier[]).forEach((tier) => {
          const productId = getStoreProductIdForTier(tier);
          const priceLabel = productId
            ? String(offeringsSnapshot.productPriceById?.[productId] ?? '').trim()
            : '';
          if (priceLabel) {
            nextPriceMap[tier] = priceLabel;
          }
        });
        setSubscriptionPriceByTier(nextPriceMap);
      } catch (error) {
        logTelemetryError(error, {
          source: 'billing_offerings_snapshot',
          userId: appUserId,
        });
      }
    });
  ...
  const purchaseSubscriptionTier = useCallback(
    async (tier: SubscriptionTier) => {
      ...
      const currentProductId =
        String(subscription?.productId ?? '').trim() ||
        (subscription.tier !== 'free' ? getStoreProductIdForTier(subscription.tier) : null);
      ...
      try {
        const result = await purchaseSubscriptionByProductId(
          currentUserId,
          productId,
          currentProductId
        );
    ...
    [applyBillingSyncResult, billingEnabled, getStoreProductIdForTier, subscription?.productId, subscription.tier]
  );
